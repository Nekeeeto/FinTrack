...
export function HeroSpendScene() {
  return (
    <div className="mx-auto mb-8 max-w-2xl px-1 sm:mb-10">
      ...
      <div className="relative mx-auto mt-5 flex h-[min(200px,42vw)] max-h-[220px] min-h-[160px] w-full max-w-md items-center justify-center sm:mt-6 sm:h-[220px] sm:max-h-none">
        {/* anillo decorativo */}
        <div
          className="pointer-events-none absolute inset-[8%] rounded-full border border-dashed border-[#5DBCD2]/20 dark:border-[#5DBCD2]/25 motion-safe:hero-orbit-spin"
          aria-hidden
        />
        <div
          className="pointer-events-none absolute inset-[18%] rounded-full border border-[#5DBCD2]/10 opacity-60 dark:opacity-80 motion-safe:hero-orbit-spin-reverse"
          aria-hidden
        />

        {/* monedas flotando detrás */}
        <span
          className="pointer-events-none absolute left-[12%] top-[18%] h-7 w-7 rounded-full bg-linear-to-br from-amber-300 to-amber-600 opacity-80 shadow-md motion-safe:hero-coin-float"
          style={{ animationDelay: "0.2s" }}
          aria-hidden
        />
        <span
          className="pointer-events-none absolute right-[14%] top-[22%] h-5 w-5 rounded-full bg-linear-to-br from-slate-200 to-slate-400 opacity-90 shadow motion-safe:hero-coin-float"
          style={{ animationDelay: "0.8s" }}
          aria-hidden
        />
        <span
          className="pointer-events-none absolute bottom-[20%] left-[20%] h-4 w-4 rounded-full bg-[#5DBCD2]/40 blur-[1px] motion-safe:hero-coin-float"
        ...
        {/* lata / alcancía */}
        <div
          className="relative z-10 motion-safe:hero-can-wobble"
        ...
        >
          <div className="relative drop-shadow-[0_20px_40px_rgba(93,188,210,0.25)] dark:drop-shadow-[0_24px_48px_rgba(0,0,0,0.45)]">
            ...
            >
              <defs>
                <linearGradient id="heroCanBody" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" className="[stop-color:theme(colors.slate.500)] dark:[stop-color:theme(colors.slate.400)]" />
                  <stop offset="35%" className="[stop-color:theme(colors.slate.300)] dark:[stop-color:theme(colors.slate.500)]" />
                  <stop offset="65%" className="[stop-color:theme(colors.slate.400)] dark:[stop-color:theme(colors.slate.600)]" />
                  <stop offset="100%" className="[stop-color:theme(colors.slate.600)] dark:[stop-color:theme(colors.slate.700)]" />
                </linearGradient>
                <linearGradient id="heroCanShine" x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor="#5DBCD2" stopOpacity="0.35" />
            ...
            {/* resplandor */}
            <div className="pointer-events-none absolute -inset-6 -z-10 rounded-full bg-[#5DBCD2]/15 blur-2xl motion-safe:hero-can-glow" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 sm:mt-6 sm:grid-cols-4 sm:gap-3">
        {BADGES.map(({ label, sub, Icon, delay, className }) => (
          <div
            key={label}
            className={`motion-safe:hero-badge-float flex flex-col items-center gap-1 rounded-2xl border px-2.5 py-2.5 text-center sm:px-3 sm:py-3 ${className}`}
